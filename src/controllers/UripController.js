...
import { connectionUrip } from "../config/Database.js";
import {
...
export const storeBulanan = async (req, res) => {
  const response = await fetch(`http://localhost:${port}/urip/pendapatan`);
...
export const storeHarian = async (req, res) => {
  try {
    ...
    const responseOmset = await fetch(
      `http://localhost:${port}/urip/pendapatan`
    );
    ...
    const response = await fetch(
      `http://localhost:${port}/urip/pendapatan/bulanan`
    );
    const dataResponse = await response.json();
...
export const run = async (req, res) => {
  try {
    ...
    if (Array.isArray(dataResponse) && dataResponse.length === 0) {
      await fetch(`http://localhost:${port}/urip/pendapatan/bulanan/store`);
    }

    await fetch(`http://localhost:${port}/urip/pendapatan/harian/store`);

    await fetch(`http://localhost:${port}/urip/pendapatan/bulanan/update`);
