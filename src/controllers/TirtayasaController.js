...
import { connectionTirtayasa } from "../config/Database.js";
import {
...
const namaKlinik = "Klinik Pratama Kosasih Tirtayasa";
const namaLab = "Laboratorium Kosasih Tirtayasa";
const bulan = getCurrentDateArray("bulan");
const tahun = getCurrentDateArray("tahun");
const idCabang = [15, 16];
...
export const getPendapatan = async (req, res) => {
  ...
  try {
    const [resultPendapatanBarangKlinik] = await connectionTirtayasa.query(
      queryGetPendapatanBarangKlinik
    );
    const [resultPendapatanJasaKlinik] = await connectionTirtayasa.query(
      queryGetPendapatanJasaKlinik
    );
    const [resultPendapatanBarangLab] = await connectionTirtayasa.query(
      queryGetPendapatanBarangLab
    );
    const [resultPendapatanJasaLab] = await connectionTirtayasa.query(
...
export const storeBulanan = async (req, res) => {
  const response = await fetch(`http://localhost:${port}/tirtayasa/pendapatan`);
...
export const storeHarian = async (req, res) => {
  try {
    ...
    const responseOmset = await fetch(
      `http://localhost:${port}/tirtayasa/pendapatan`
    );
    ...
    const response = await fetch(
      `http://localhost:${port}/tirtayasa/pendapatan/bulanan`
    );
    const dataResponse = await response.json();
...
export const run = async (req, res) => {
  try {
    ...
    if (Array.isArray(dataResponse) && dataResponse.length === 0) {
      await fetch(
        `http://localhost:${port}/tirtayasa/pendapatan/bulanan/store`
      );
    }

    await fetch(`http://localhost:${port}/tirtayasa/pendapatan/harian/store`);

    await fetch(`http://localhost:${port}/tirtayasa/pendapatan/bulanan/update`);
